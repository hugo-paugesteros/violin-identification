...
# print(duration_sum.sum(axis='player'))
...
ax3.matshow(duration_sum.to_numpy(), aspect='auto')
ax3.set_xticks(np.arange(len(set(df['player']))), labels=list(set(df['player'])))
ax3.set_yticks(np.arange(len(set(df['violin']))), labels=list(set(df['violin'])))
...
plt.savefig('figures/class_weights.svg', bbox_inches='tight', dpi=300)
plt.show()